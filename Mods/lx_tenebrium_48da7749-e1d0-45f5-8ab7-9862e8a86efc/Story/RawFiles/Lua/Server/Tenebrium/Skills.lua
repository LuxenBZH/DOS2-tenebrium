...
local function HitAnalysis(target, instigator, damage, handle)
    ...
    if status.DamageSourceType == "Attack" or status.SkillId ~= "" then
        if target:GetStatus("TEN_PARANOIA") and target.MyGuid ~= instigator.MyGuid and CharacterIsAlly(target.MyGuid, instigator.MyGuid) == 1 then
            ApplyStatus(target.MyGuid, "FEAR", 6.0, 1, instigator.MyGuid)
        end
    end
end
...
Ext.Events.BeforeStatusApply:Subscribe(function(e)
    if e and e.Status and Ext.Utils.IsValidHandle(e.Status.StatusSourceHandle) and Osi.HasActiveStatus(Ext.Entity.GetGameObject(e.Status.StatusSourceHandle).MyGuid, "TEN_HEX") == 1 and e.Status.LifeTime >= 12.0 then
