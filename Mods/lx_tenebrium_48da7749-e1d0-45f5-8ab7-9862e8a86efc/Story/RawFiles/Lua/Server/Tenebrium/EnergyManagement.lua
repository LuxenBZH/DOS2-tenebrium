...
local function UpdateTE(character, event)
    ...
    -- Sigils influence
    if HasActiveStatus(character, "TEN_SIGIL_OBSCURA") == 1 then
        if IsTagged(character, "TEN_Sigil_TurnStart") == 1 then
            ClearTag(character, "TEN_Sigil_TurnStart")
        else
            if newValue > 0 then
                newValue = newValue + 5
            end
        end
    elseif HasActiveStatus(character, "TEN_SIGIL_AURORA") == 1 then
        if IsTagged(character, "TEN_Sigil_TurnStart") == 1 then
            ClearTag(character, "TEN_Sigil_TurnStart")
        else
            if newValue > 0 then
                newValue = math.ceil(newValue * 0.5)
            end
        end
    end
...
function AddTEnergy(character, amount)
    -- local oldValue = GetVarInteger(character, "SRP_TEnergy")
