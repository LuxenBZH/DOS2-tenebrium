...
local function SetShadowBarAmount(ui, call, ...)
    ...
    if not character then
        Ext.Print("Couldn't get character GUID from the sheet for TE UI management!")
        return
    end
    local amount = character:GetCustomStat(StatTE.Id) or 0
    ...
    local ti = character:GetCustomStat(StatTI.Id) or 0
    if amount > ti then
