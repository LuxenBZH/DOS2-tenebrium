...
local function TenebriumSkillDisplayRequirement(character, skill, tooltip)
    ...
    for i,element in pairs(tooltip:GetElements("SkillRequiredEquipment")) do
        ...
        local _,_,amount = string.find(element.Label, "tag SRP_TE_R(%d+)")
        if amount ~= nil then
            -- Ext.Print("Requirement not met")
            requirementMet = false
            element.Label = SubstituteString(Ext.GetTranslatedStringFromKey("TE_Requirement"), amount)
        end
    ...
    for i, requirement in pairs(skill.Requirements) do
        if requirement.Requirement == "Tag" then
            ...
            local _,_,amount = string.find(requirement.Param, "SRP_TE_R(%d+)") --Ty LaughingLeader
            if amount ~= nil then
                local element = {
                    Label = SubstituteString(Ext.GetTranslatedStringFromKey("TE_Requirement"), amount),
                    RequirementMet = true,
                    Type = "SkillRequiredEquipment"
                }
                tooltip:AppendElementAfter(element, "SkillRequiredEquipment")
...
local function OnStatTooltip(character, stat, tooltip)
    ...
    statsDescription.Label = "How active is the Tenebrium Infusion. Increase with missed and resisted attacks, by receiving a critical hit or when being incapacitated. The stronger the infusion is, the more energy it will generate.<br>Energy growth can increase the infusion over time."
